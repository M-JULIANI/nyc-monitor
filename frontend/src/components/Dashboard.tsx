          ...
          <div className="max-w-7xl mx-auto space-y-4 pb-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <div className="card p-3">
                <h3 className="text-sm font-semibold mb-1 text-white">Active Alerts</h3>
                <p className="text-xl font-bold text-primary">{stats.active}</p>
                <p className="text-xs text-zinc-300">{stats.total} total</p>
              </div>

              <div className="card p-3">
                <h3 className="text-sm font-semibold mb-1 text-white">Critical</h3>
                <p className="text-xl font-bold text-red-400">{stats.critical}</p>
                <p className="text-xs text-zinc-300">Immediate attention</p>
              </div>

              <div className="card p-3">
                <h3 className="text-sm font-semibold mb-1 text-white">High Priority</h3>
                <p className="text-xl font-bold text-orange-400">{stats.high}</p>
                <p className="text-xs text-zinc-300">Urgent response</p>
              </div>

              <div className="card p-3">
                <h3 className="text-sm font-semibold mb-1 text-white">Data Sources</h3>
                <p className="text-xl font-bold text-yellow-400">{Object.keys(stats.bySource).length}</p>
                <p className="text-xs text-zinc-300">Reddit, 311, Twitter</p>
              </div>
            </div>
                ...
                {isLoadingReports ? (
                  ...
                  <div className="py-8 text-center">
                    ...
                    <p className="text-sm text-zinc-500 mt-2">
                      Investigation reports will appear here after alerts are investigated and completed
                    </p>
                  </div>
                ) : (
                      ...
                      return (
                        <div key={alert.id} className="bg-zinc-800 rounded-lg p-4 border border-zinc-700">
                          <div className="flex items-start justify-between mb-3">
                            ...
                            <span className="text-xs px-2 py-1 rounded bg-green-900 text-green-300">Resolved</span>
                          </div>

                          <h4 className="text-white font-medium text-sm mb-2 line-clamp-2">{alert.title}</h4>
